...
export default function Menu() {
  // Legge il menu da Firebase — fallback ai dati statici
  const { data: menuData } = useFirestore<{ categorie: MenuCategory[]; coperto: string }>(
    'menu',
    { categorie: staticMenu, coperto: '2' }
  );

  const categorie = (menuData.categorie && menuData.categorie.length > 0)
    ? menuData.categorie
    : staticMenu;
  const coperto = menuData.coperto ?? '2';

  const [active, setActive] = useState<string>(categorie[0]?.id ?? '');
  const [selected, setSelected] = useState<MenuItem | null>(null);

  const current = categorie.find((c) => c.id === active) ?? categorie[0];
  const accentColors = [
    "bg-mustard", "bg-brick text-kraft-light", "bg-teal text-kraft-light",
    "bg-hotpink text-kraft-light", "bg-lemon", "bg-grape text-kraft-light",
  ];

  return (
    <>
      <section className="relative border-b-2 border-mocha bg-kraft-light">
        <img src={doodleKid} alt="" aria-hidden="true"
        ...
        <img src={doodleDog} alt="" aria-hidden="true"
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-16 lg:px-10">
          <span className="stamp text-brick">la carta · coperto {coperto}€</span>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-10">
        {current && (
          <div key={current.id} className="relative reveal">
            <img src={doodleDancer} alt="" aria-hidden="true"
            ...
            <div className="mb-10 flex items-end justify-between gap-6 border-b-2 border-mocha pb-5">
              <div>
                <span className="font-mono text-xs uppercase tracking-[0.3em] text-brick">
                  #{String(categorie.indexOf(current) + 1).padStart(2, "0")}
                </span>
      ...
      {/* Modal */}
      {selected && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-mocha/70 p-4"
          onClick={() => setSelected(null)}>
          <div className="relative max-w-lg border-2 border-mocha bg-kraft-light p-8 shadow-hard animate-scale-in"
            onClick={(e) => e.stopPropagation()}>
            <button aria-label="Chiudi" onClick={() => setSelected(null)}
